 ...
 * @date 2020-11-19
 */
public class MyPathSimplification {
     ...
     */
    public static PointList simplify(RoutePath routePath, DouglasPeucker douglasPeucker, boolean enableInstructions) {
        final PointList pointList = routePath.getPointList();
        List<Partition> partitions = new ArrayList<>();
        // todo: maybe this code can be simplified if path details and instructions would be merged, see #1121
        if (enableInstructions) {
            final InstructionList instructions = routePath.getInstructions();
        ...
        for (final Map.Entry<String, List<PathDetail>> entry : routePath.getPathDetails().entrySet()) {
        ...
        simplify(routePath.getPointList(), partitions, douglasPeucker);
        assertConsistencyOfPathDetails(routePath.getPathDetails());
        if (enableInstructions) {
            assertConsistencyOfInstructions(routePath.getInstructions(), routePath.getPointList().size());
        }
        return pointList;
    }

    public static void simplify(PointList pointList, List<Partition> partitions, DouglasPeucker douglasPeucker) {
        new MyPathSimplification(pointList, partitions, douglasPeucker).simplify();
    }

    private MyPathSimplification(PointList pointList, List<Partition> partitions, DouglasPeucker douglasPeucker) {
